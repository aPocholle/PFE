...
typedef struct {
    ...
    XC_grav_uio_map maps[ MAX_UIO_MAPS ];
} XC_grav_uio_info;

/***************** Variable Definitions **************************************/
static XC_grav_uio_info uio_info;
...
static int uio_info_read_name(XC_grav_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XC_grav_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XC_grav_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XC_grav_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XC_grav_Initialize(XC_grav *InstancePtr, const char* InstanceName) {
	XC_grav_uio_info *InfoPtr = &uio_info;
